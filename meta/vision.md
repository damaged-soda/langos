...
本文件描述 workspace-docs 作为“自然语言驱动的 AI 编程操作系统”时的愿景、范围和分层定位，帮助人类与 AI 在同一套规则下协作。
...
## 3. 分层定位与边界
- **治理层（愿景/原则/迭代方式）**  
  - 放置位置：`langos/meta/`（本文件、conventions、迭代说明等）。  
  - 包含：愿景、基本原则、分层说明、迭代/变更策略。  
  - 不包含：具体业务需求、具体协议实现细节。
- **接口与执行层（规范/协议/运行时）**  
  - 放置位置：`langos/runtime/`（guidelines、runtime、protocols/*）。  
  - 包含：对话与行为规范、自然语言路由与确认方式、协议定义与索引、可复用模板/checklist/片段库。  
  - 不包含：具体业务需求或仓库特有信息。
- **业务资产层（仓库索引/需求/项目/领域知识）**  
  - 放置位置：`workspace/repos/`、`workspace/projects/`、`workspace/domain/`、`workspace/specs/` 等。  
...
- 记录方式：当前阶段不强制 ADR，但重要决策可择机写入 `workspace/adr/` 便于长期追踪。
