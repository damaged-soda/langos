...
- 启动阶段仅校验配置的 `doc_root` 是否存在，不预读取或暴露内容；doc_root 缺省时可按最小集继续，选择治理/业务模式且需要文档时，再按需读取 `doc_root` 下文档。
- 仅在改“OS”/规范/协议时加载：`runtime/conventions.md`，其余背景文档可参考文档仓（如 `../langos-docs/repos/langos/`），但运行时不依赖其内容。
...
本文件描述“接口与执行层”如何把自然语言请求路由到协议，并在需求→方案→编码→评审四个环节保持可控、可审计、可复用。治理/自举任务视为业务流程的特例，沿用相同阶段，仅增加文档来源与热改风险的提示与守卫。
...
5) 如修改协议/路由/规范（内核模式），可在文档仓参考 `governance.md` 等背景说明，写 spec → 草拟协议 → 按确认的方案落盘并更新 index（必要时可手动对话模拟关键路径）；运行时仍可在未读取 docs 时按最小集推进确认。
