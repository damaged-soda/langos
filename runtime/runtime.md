...
5) 如修改协议/路由/规范（内核模式），可在文档仓按需参考蓝图（如 `blueprints/vision.md`）做背景；写 spec → 草拟协议 → 按确认的方案落盘并更新 index（必要时可手动对话模拟关键路径）；运行时仍可在未读取 docs 时按最小集推进确认。
...
## 7. 更新与扩展
- 新增或调整流程时，按治理流程完成需求与方案后直接修改协议文件，并同步 `index.yaml`。  
