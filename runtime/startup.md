...
> 供 codex/AI 在 `work/` 目录加载 langos 成功后使用。先校验文档目录，再进入模式选择。
...
- 启动时复述：已处于 langos 运行时，治理/运行时在本仓，业务文档不在本仓；治理时将 langos 本身视为项目按标准流程推进，运行时最小集无需依赖 docs。
...
2) 治理/改协议：调整规范/协议/分层（将 langos 视为目标项目，按需求→方案→编码→评审推进，进入后按需读取文档）
...
- “治理/改协议”：治理=业务流程特例，将 langos 视为目标项目，进入后按需参考 `langos-docs/*`（如蓝图/规格）；未指定 doc_root 时，先用最小集沟通，需要读文档时再提示指定。  
- “业务开发”：进入该模式后，再判断业务文档目录：  
  - 若已由配置/用户指定，则使用该目录；  
  - 若未指定，提示指定业务目录；文档结构以 doc_root 所在文档库的规范为准（如 langos-docs/meta/conventions.md）；  
...
### 2. 内核治理模式 (Kernel Mode)
- **目标**：迭代 LangOS 本身（修改协议、规范或文档），复用业务开发的需求→方案→编码→评审流程。
- **工作区根目录**：**`langos/`（本仓根目录）**。
- **逻辑映射**：
    - 把 `langos/` 视为“当前 Repo”。
    - 把 `doc_root`（来自配置或指定，例如 `langos-docs/`，可选）视为“业务/背景文档库”，进入治理后按需读取；未指定时按最小集继续。
